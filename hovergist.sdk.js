...
(function (global) {
  ...
  function showActiveBadge() {
    ...
    activeBadgeEl.innerHTML = `
      ...
      <span>HoverGist Active — hover a section, press G</span>`;
  ...
  function onKeyDown(e) {
    // Alt → toggle activate/deactivate
    ...
    // G → generate gist on currently hovered element (no click needed)
    if ((e.key === 'g' || e.key === 'G') && isActive && !e.repeat) {
      if (!currentElement) return;
      e.preventDefault();
      fetchGist(currentElement);
    }
  }
  ...
  // Click only highlights the element — does NOT generate gist (use G key instead)
  function onClick(e) {
    if (!isActive) return;
    ...
    highlightElement(el);
    // Note: fetchGist intentionally removed — press G to generate gist
  }
  ...
  const HoverGist = {
    ...
    init(userConfig = {}) {
      ...
      console.info(
        `%c HoverGist SDK v1.1.0 %c Initialized | Press ${config.triggerKey} to activate, G to gist `,

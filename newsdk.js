...
(function (global) {
  ...
  const OB = (function () {
    ...
    function obTypeText(text, speed) {
      return new Promise(res => {
    ...
    // ── Main animation loop ──────────────────────────────────────────
    async function obRunAnimation(triggerKey) {
      if (!obRunning) return;
      ...
      await obTypeText('Neural networks learn by adjusting weights — no rules, just pattern recognition at massive scale.', 22);
      if (!obRunning) return;
      ...
      if (obRunning) obRunAnimation(triggerKey);
    }
    ...
    function show(opts) {
      ...
      document.getElementById('hgo-ob-replay').addEventListener('click', function () {
        obRunning = false;
        setTimeout(() => { obRunning = true; obRunAnimation(obConfig.triggerKey); }, 60);
      });
      ...
      setTimeout(function () { obRunAnimation(obConfig.triggerKey); }, 600);
    }
  ...
  const HoverGist = {
    ...
    destroy()  { deactivate(); document.removeEventListener('keydown', onKeyDown, true); },
  };
